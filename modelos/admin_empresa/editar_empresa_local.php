 ...
 * Obtiene el grupo del usuario y crea los registros faltantes en sec_groups_app
 */
function sincronizarPermisosGrupo($pdo, $masterDb)
{
    ...
    try {
        // Verificar que la tabla sec_groups_app existe
        ...
        // Obtener todos los usuarios logeados en sesión
        if (!isset($_SESSION['id_login']) || $_SESSION['id_login'] <= 0) {
            return $result; // Sin error, simplemente retornar
        }

        // Obtener el id_grupo del usuario desde sec_users
        ...
        // Obtener todas las aplicaciones (sec_apps)
        ...
        // Para cada aplicación, verificar si existe permiso en sec_groups_app
        foreach ($apps as $appId) {
            ...
            if (!$exists) {
                // Crear registro de permiso por defecto (acceso=on)
                try {
...
if ($action === 'delete') {
    ...
    try {
        ...
        // Validar confirmación (debe escribir el RUC)
        if ($confirmacion !== $empresa['ruc']) {
        ...
        if ($checkDb) {
            ...
            if ($returnCode === 0 && file_exists($backupFile) && filesize($backupFile) > 100) {
                ...
                // Intentar backup alternativo con PHP
            ...
            // SOLO eliminar la base de datos si el backup fue exitoso
            if ($backupSuccess) {
        ...
        // Eliminar registro de empresa (solo si backup fue exitoso o no había DB)
    ...
    <script>
        ...
        function empresaForm() {
            ...
            return {
                ...
                async confirmarEliminar() {
                    ...
                    confirmBtn.addEventListener('click', async () => {
                        ...
                        window.updateProgress({
                            progress: 10,
                            addLog: 'Validando datos de empresa...',
                            logType: 'loading'
                        });
                        ...
                        try {
                            ...
                            window.updateProgress({
                                progress: 30,
                                addLog: 'Creando backup de base de datos...',
                                logType: 'loading'
                            });
                            ...
                            if (data.success) {
                                ...
                                if (data.backup_file) {
                                    window.updateProgress({
                                        progress: 60,
                                        addLog: `💾 Archivo: ${data.backup_file}`,
                                        logType: 'success'
                                    });
                                    await this.sleep(200);
                                }
                                ...
                                window.updateProgress({
                                    progress: 70,
                                    addLog: 'Eliminando base de datos...',
                                    logType: 'loading'
                                });
                                await this.sleep(300);
                                ...
                                window.updateProgress({
                                    progress: 95,
                                    addLog: 'Eliminando registro de empresa...',
                                    logType: 'loading'
                                });
                                await this.sleep(200);
                                ...
                                // Mostrar mensaje final con botón aceptar
                                window.updateProgress({
                                    addLog: '✅ Empresa eliminada correctamente',
                                    logType: 'success'
                                });

                                // Agregar botón Aceptar al modal de progreso
                                const progressModal = document.getElementById('progressModal');
                                if (progressModal) {
                                    const footer = progressModal.querySelector('.progress-footer') || progressModal.querySelector('.flex.justify-end');
                                    if (!footer) {
                                        const modalContent = progressModal.querySelector('.bg-white, .dark\\:bg-slate-800') || progressModal.firstElementChild?.firstElementChild;
                                        if (modalContent) {
                                            const btnContainer = document.createElement('div');
                                            btnContainer.className = 'flex justify-center mt-4 pt-4 border-t border-slate-200 dark:border-slate-700';
                                            btnContainer.innerHTML = `
                                                <button id="acceptDeleteBtn" class="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors">
                                                    <i class="fas fa-check mr-2"></i>Aceptar
                                                </button>
                                            `;
                                            modalContent.appendChild(btnContainer);

                                            document.getElementById('acceptDeleteBtn').addEventListener('click', () => {
                                ...
                                window.updateProgress({
                                    progress: 100,
                                    log: data.error || 'Error al eliminar',
                                    logType: 'error'
                                });

                                // Agregar botón Cerrar en caso de error
                                const progressModal = document.getElementById('progressModal');
                                if (progressModal) {
                                    const modalContent = progressModal.querySelector('.bg-white, .dark\\:bg-slate-800') || progressModal.firstElementChild?.firstElementChild;
                                    if (modalContent) {
                                        ...
                                        btnContainer.className = 'flex justify-center mt-4 pt-4 border-t border-slate-200 dark:border-slate-700';
                                        btnContainer.innerHTML = `
                                            <button id="closeErrorBtn" class="px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors">
                            ...
                            // Agregar botón Cerrar en caso de error de conexión
                            const progressModal = document.getElementById('progressModal');
                            if (progressModal) {
                                const modalContent = progressModal.querySelector('.bg-white, .dark\\:bg-slate-800') || progressModal.firstElementChild?.firstElementChild;
                                if (modalContent) {
                                    ...
                                    btnContainer.className = 'flex justify-center mt-4 pt-4 border-t border-slate-200 dark:border-slate-700';
                                    btnContainer.innerHTML = `
                                        <button id="closeErrorBtn2" class="px-6 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-medium transition-colors">
                                            <i class="fas fa-times mr-2"></i>Cerrar
                                        </button>
                                    `;
                                    modalContent.appendChild(btnContainer);

                                    document.getElementById('closeErrorBtn2').addEventListener('click', () => {
                                        progressModal.remove();
                                    });
                                }
                            }
                ...
                cerrarModal() {
                    // Cerrar el iframe del menú principal
                    if (window.parent && window.parent !== window) {
                        try {
                            // Cerrar el appFrame del menú
                            const appFrame = window.parent.document.getElementById('appFrame');
                            if (appFrame) {
                                appFrame.src = 'about:blank';
                            }
                        } catch (e) {
                            console.log('Error cerrando iframe:', e);
                            window.history.back();
                        }
                    } else {
                        window.history.back();
