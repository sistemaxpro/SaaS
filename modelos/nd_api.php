...
/**
 * Actualiza el estado de una ND en la base de datos local
 */
function updateNDStatus($cdc, $estado, $mensaje)
{
    if (!$cdc)
        return false;
    ...
    try {
        ...
        if ($dbName) {
            $sql = "UPDATE $dbName.de_nd SET est_res = :estado, msg_res = :msg, updated_at = NOW() WHERE id_sifen = :cdc";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([':estado' => $estado, ':msg' => $mensaje, ':cdc' => $cdc]);
            return true;
        }
    } catch (Exception $e) {
        error_log("Error updating ND status: " . $e->getMessage());
    }
    return false;
}

/**
 * Inserta una nueva nota de débito en la base de datos local
 */
function insertNotaDebitoLocal($payload, $sifenResponse, $conceptos)
{
    $masterDb = $_SESSION['dbu'] ?? 'serproc1';
    $dbHost = $_SESSION['server'] ?? '168.231.95.50';
    $dbUser = $_SESSION['user'] ?? 'sistemax';
    ...
    try {
        ...
        // Verificar si ya existe una ND para este CDC asociado y ndoc (evitar duplicados)
        $stmtCheck = $pdo->prepare("SELECT id, est_res, id_sifen FROM $dbName.de_nd WHERE cdc_asociado = :cdc AND ndoc = :ndoc LIMIT 1");
        $stmtCheck->execute([':cdc' => $cdcAsociado, ':ndoc' => $ndoc]);
        $existing = $stmtCheck->fetch(PDO::FETCH_ASSOC);

        if ($existing) {
            // Ya existe, actualizar en lugar de insertar
            $stmtUpdate = $pdo->prepare("UPDATE $dbName.de_nd SET 
            ...
            error_log("ND_API: ND existente actualizada (ID: {$existing['id']})");
            return $existing['id'];
        }

        $sqlNC = "INSERT INTO $dbName.de_nd (
        ...
        $sqlItems = "INSERT INTO $dbName.de_nd_items (nota_debito_id, codigo, descripcion, precio, cantidad, tasa_iva, descuento, unidad_medida) VALUES (:id_nc, :codigo, :descripcion, :precio, :cantidad, :tasa_iva, :descuento, :unidad_medida)";
        ...
        error_log("Error insertando ND: " . $e->getMessage());
        return false;
    }
}
...
$timbradoRecibido = (string) $get('timbrado', $payload, '');

// Debug: Log valores recibidos
error_log("ND_API - Payload recibido: timbrado='$timbradoRecibido', modo='$modo', ruc='$ruc', cdc='$cdcAsociado'");
...
if ($timbradoRecibido === '' || $timbradoRecibido === '0') {
    echo json_encode(['success' => false, 'message' => "Timbrado inválido o no configurado. Ambiente: $modo, Timbrado recibido: '$timbradoRecibido'"]);
    exit;
}

// Debug: Log del cert_path recibido
error_log("ND_API - cert_path recibido: '$certPath', is_file: " . (is_file($certPath) ? 'true' : 'false'));
...
try {
    ...
    // Configurar CSC e IDC para el QR hash
    ...
    if ($action === 'anular') {
        ...
        if ($respuesta['status'] === 'ok' && !empty($respuesta['response'])) {
            ...
            updateNDStatus($cdcAsociado, $finalEstado, $msgRes);
    ...
    $timbradoEnviar = (string) $get('timbrado', $payload, '0');
    $fecTimbradoEnviar = (string) $get('fec_timbrado', $payload, $hoy);

    // Log detallado del timbrado a archivo local
    $logMsg = date('Y-m-d H:i:s') . " - ND_API - Timbrado a enviar: '$timbradoEnviar', Fecha: '$fecTimbradoEnviar', ndoc: '$ndoc'\n";
    file_put_contents(__DIR__ . '/nd_api_debug.log', $logMsg, FILE_APPEND);

    $factura = new \sifen\Factura(\sifen\DTE::ND, [
        'emisor' => new \sifen\Emisor($emisorData),
        'receptor' => new \sifen\Receptor($receptorData),
        'conceptos' => $conceptos,
        'ndoc' => $ndoc,
        'timbrado' => $timbradoEnviar,
        'fec_timbrado' => $fecTimbradoEnviar,
        'cod_establecimiento' => (string) $get('cod_establecimiento', $payload, '001'),
        'cod_expedicion' => (string) $get('cod_expedicion', $payload, '001'),
    ...
    error_log("ND_API - CDC generado: $cdcGenerado");
    error_log("ND_API - Timbrado en factura: " . ($factura->timbrado ?? 'NULL'));
    ...
    file_put_contents(__DIR__ . '/nd_xml_debug.xml', $xmlDebugStr);
    error_log("ND_API - XML generado guardado en nd_xml_debug.xml");
    ...
    // Log para debug
    error_log("ND_API - Respuesta SIFEN: " . json_encode($respuesta));
    error_log("ND_API - Timbrado enviado: $timbradoEnviar, Modo: $modo");
    ...
    if (isset($respuesta['status']) && $respuesta['status'] === 'ok' && !empty($respuesta['response'])) {
        ...
        if (!empty($protConsLote)) {
            ...
            for ($retry = 1; $retry <= $maxRetries; $retry++) {
                ...
                if (!empty($resLote['response'])) {
                    ...
                    file_put_contents(__DIR__ . '/nd_sifen_response.xml', $resLote['response']);
                    error_log("ND_API - Respuesta SIFEN guardada en nd_sifen_response.xml (intento $retry)");
                    ...
                    if ($nodesCod->length > 0 && $nodesCod->item(0)->nodeValue === '0361') {
                        ...
                        error_log("ND_API - Lote aún en procesamiento (intento $retry/$maxRetries)");
                        continue;
                    }
                    ...
                    if ($codigoError === '1101') {
                        $mensajeFinal = "<strong>Error $codigoError</strong><br><br>" .
                            "Timbrado <strong>$timbradoEnviar</strong> no está autorizado para emitir <strong>Nota de Débito</strong>.<br><br>" .
                            "<em>Solución:</em> Ingrese al portal SIFEN y habilite el tipo de documento 'Nota de Débito Electrónica' para este timbrado.";
        ...
        $dbInserted = insertNotaDebitoLocal($payload, $responseData, $conceptosRaw);

        // Mensaje con información del timbrado para debugging
        $msgExtra = " [Timbrado: $timbradoEnviar, Fecha: $fecTimbradoEnviar]";

        // Si fue rechazado, mostrar como error
        if ($estadoFinal === 'Rechazado') {
            echo json_encode([
                'success' => false,
                'message' => $mensajeFinal . $msgExtra,
                'data' => array_merge($responseData, ['timbrado_enviado' => $timbradoEnviar, 'fec_timbrado' => $fecTimbradoEnviar])
            ]);
        } else {
            echo json_encode([
                'success' => true,
                'message' => 'ND procesada' . ($dbInserted ? ' y guardada.' : '.') . $msgExtra,
                'data' => array_merge($responseData, ['db_inserted' => $dbInserted, 'timbrado_enviado' => $timbradoEnviar])
            ]);
        }
    } else {
        $rucEmisor = $emisorData['ruc'] ?? $ruc ?: 'N/A';
        echo json_encode([
            'success' => false,
            'message' => ($modo === 'test' ? 'TEST' : 'PROD') . " - " . ($respuesta['error'] ?? 'Error desconocido') . " [RUC: $rucEmisor, Timbrado: $timbradoEnviar]",
            'data' => $respuesta
        ]);
    }
} catch (Exception $e) {
