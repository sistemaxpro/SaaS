...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'get_next_nd') {
    ...
    try {
        ...
        // Get max sequence for current timbrado for ND
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'search_facturas') {
    ...
    // Add filters only if query is present
    if ($q !== '') {
        $sqlSearch .= "
        AND (
    ...
    $sqlSearch .= " ORDER BY fv.id_factura DESC LIMIT 50";

    $logs[] = "SQL 3 (Search): " . str_replace(["\t", "\n", "\r"], " ", $sqlSearch) . " [Params: " . json_encode($params) . "]";
...
// Query for Both NC and ND
// We can use a UNION ALL to fetch both if no explicit type is filtered, or just fetch both by default for the unified grid.
$masterDb = $_SESSION['dbu'] ?? 'serproc1';
...
$ncRows = array_map(function ($row) {
    ...
    return [
        ...
        'msg_res' => $r['msg_res'] ?? '',
        'cdc' => $r['cdc'],
        'tipo_documento' => $r['tipo_documento'] ?? 'NC', // Default fallback
    ];
}, $rows);
...
<body>
    ...
    <div class="page-header">
        ...
        <div class="toolbar">
            ...
            <div class="btn-chip" id="btnInitNew">
    ...
    <div id="modalNC" class="modal-overlay">
        <div class="modal-content">
            <div class="modal-header" style="border-bottom: 3px solid var(--success); transition: border-color 0.3s ease;">
                <h3 style="margin:0; display:flex; align-items:center; gap:10px;">
                    <span id="modalTitle">Nueva Nota de Crédito</span>
                </h3>
                <div style="display:flex; gap:15px; align-items:center; background:#f1f5f9; padding:8px 20px; border-radius:20px; border:1px solid #ccc; z-index:10;">
                    <label style="cursor:pointer; display:flex; align-items:center; gap:6px; font-weight:bold; font-size:14px; color:#333;">
                        <input type="radio" name="docType" value="NC" checked style="transform:scale(1.2);"> 
                        <span style="color:#16a34a">Nota Crédito</span>
                    </label>
                    <label style="cursor:pointer; display:flex; align-items:center; gap:6px; font-weight:bold; font-size:14px; color:#333;">
                        <input type="radio" name="docType" value="ND" style="transform:scale(1.2);"> 
                        <span style="color:#2563eb">Nota Débito</span>
                    </label>
                </div>
                <button id="btnCloseModal" title="Cerrar"
                    style="background:none; border:none; color:var(--text); font-size:1.5rem; cursor:pointer;">
            ...
            <div class="modal-body">
                ...
                <div class="nc-panes">
                    <div class="nc-pane">
                        <h3 style="margin-top:0;">1. Ítems del Documento (Arrastre aquí)</h3>
    ...
    <script>
        ...
        // -- STATE MANAGEMENT --
        let currentDocType = 'NC'; // 'NC' or 'ND'
        ...
        function setDocType(type) {
            ...
            const targetColor = type === 'NC' ? 'var(--success)' : '#3b82f6';
            
            // Update UI
            if (modalTitle) modalTitle.innerText = type === 'NC' ? 'Nueva Nota de Crédito' : 'Nueva Nota de Débito';
            document.querySelector('.modal-header').style.borderBottomColor = targetColor;
            
            // Update Motivos
            updateMotivoOptions(type);

            // Fetch sequence if open
            if (modal.classList.contains('open') && !document.getElementById('ndocInput').readOnly) {
                 fetchSequence(type);
            }
        }

        function updateMotivoOptions(type) {
            const select = document.getElementById('motivoInput');
            if (!select) return;
            ...
            if (type === 'NC') {
                options = [
                    {val:1, text:'DEVOLUCIÓN Y AJUSTE DE PRECIOS'},
                    {val:2, text:'DEVOLUCIÓN'},
                    {val:3, text:'DESCUENTO'},
                    {val:4, text:'BONIFICACIÓN'},
                    {val:5, text:'CRÉDITO INCOBRABLE'},
                    {val:6, text:'RECUPERO DE COSTO'},
                    {val:7, text:'RECUPERO DE GASTO'},
                    {val:8, text:'AJUSTE DE PRECIO'}
                ];
            } else { // ND
                options = [
                    {val:1, text:'DIFERENCIA DE PRECIO'},
                    {val:2, text:'DIFERENCIA EN CANTIDAD'},
                    {val:3, text:'INTERESES'},
                    {val:4, text:'RECUPERO DE GASTOS'}
                ];
            }
        ...
        const columnDefs = [
            ...
            { 
                 ...
                 cellRenderer: p => {
                     const val = p.value;
                     const color = val === 'NC' ? '#22c55e' : '#3b82f6'; // Green for NC, Blue for ND
                     return `<span style="font-weight:bold; color:${color}">${val}</span>`;
                 }
            },
            { headerName: 'Fecha', field: 'fecha', width: 140, sortable: true },
            { headerName: 'Nro Documento', field: 'nro_factura', width: 150 },
        ...
        const gridOptions = {
            ...
            getContextMenuItems: (params) => {
                ...
                const type = d.tipo_documento; // 'NC' or 'ND'
                const typeLabel = type === 'ND' ? 'Nota de Débito' : 'Nota de Crédito';
                
                const result = [
                    {
                        name: 'Editar ' + typeLabel,
                        icon: '<i class="fa-solid fa-pen-to-square"></i>',
                        action: () => {
                            openEditModal(d);
                        }
                    },
                    (d.estado_electronico === 'Rechazado' || d.estado_electronico === 'Error') ?
                        {
                            name: 'Eliminar Registro (Local)',
                            icon: '<i class="fa-solid fa-trash" style="color:#ef4444;"></i>',
                            action: () => {
                                eliminarDoc(d.id_factura, type);
                            }
                        } :
                        {
                            name: 'Anular ' + typeLabel,
        ...
        async function fetchSequence(type) {
            try {
                const action = type === 'ND' ? 'get_next_nd' : 'get_next_nc';
        ...
        // New Doc Button Logic (Direct Open)
        const btnInitNew = document.getElementById('btnInitNew');
        if (btnInitNew) {
            btnInitNew.addEventListener('click', () => {
                openNewModal('NC'); // Default to NC, user can switch inside
            });
        }
        ...
        if (document.getElementById('invoiceSelect')) {
            tomSelectInstance = new TomSelect('#invoiceSelect', {
                ...
                searchField: 'text',
                maxItems: 1,
                preload: true, // Auto load on click
                placeholder: 'Escriba Nro Factura o Cliente...',
                load: function (query, callback) {
                    // if (!query.length) return callback(); // Allow empty load
        ...
        function eliminarDoc(id, type) {
            const tableMap = { 'NC': 'de_nc', 'ND': 'de_nd' }; // Logic server side
            const action = type === 'ND' ? 'delete_nd' : 'delete_nc'; // Server needs to handle delete_nd too
            
            Swal.fire({
                title: '¿Eliminar Registro Local?',
                text: `Esto borrará la ${type === 'ND' ? 'Nota de Débito' : 'Nota de Crédito'} de su base de datos local. NO se enviará nada a SIFEN. Esta acción no se puede deshacer.`,
                icon: 'warning',
        ...
        async function anularDoc(data) {
            const type = data.tipo_documento || 'NC';
            
            if (!data.cdc) return Swal.fire('Error', 'Este documento no tiene un CDC válido.', 'error');

            const { value: motivo } = await Swal.fire({
                title: `Anular ${type === 'ND' ? 'Nota de Débito' : 'Nota de Crédito'}`,
                text: `¿Está seguro que desea anular el documento ${data.nro_factura}?`,
                input: 'select',
                inputOptions: {
            ...
            if (motivo) {
                ...
                try {
                    ...
                    console.log(`Payload anular ${type}:`, payload);

                    // Use correct debug/api endpoint. Usually anular.php logic is shared or we use the specific API
                    // Assuming 'nc_debug.php' handles anulacion globally, or we need to point to specific.
                    // If nd.php has anulacion logic, we should use that. 
                    // Actually, anulacion is usually a separate generic event in SIFEN (Eventos).
                    // I'll stick to 'nc_debug.php' for now as it contains the cancellation logic seen in previous turns.
                    
                    const cancelUrl = type === 'ND' ? API_ND_URL : API_NC_URL;
                    ...
                    if (d.success) {
                        const info = [];
                        const statusMsg = d.message || 'Solicitud procesada correctamente.';
                        info.push(`Estado: ${statusMsg}`);
                        
                        // Marcar en la grilla como Anulada
                        markRowAsAnulada(data.cdc, statusMsg);

                        Swal.fire({
                            title: 'Anulación enviada',
                            html: `<div style="text-align:left;line-height:1.5">${info.map(i => `<div>${i}</div>`).join('')}</div>`,
                            icon: 'success'
                        });
                    } else {
                        const rechazoCode = d.codigo_rechazo || d.rechazo_codigo || d.codigo || d.code || d.cod_rechazo || d.cod_respuesta;
                        const extra = rechazoCode ? `<br><small>Código de rechazo: ${rechazoCode}</small>` : '';
                        Swal.fire({
                            title: 'Rechazado',
                            html: `${d.message || 'La anulación fue rechazada.'}${extra}`,
                            icon: 'error'
                        });
                    }
                } catch (e) {
        ...
        document.getElementById('btnEnviarNC').addEventListener('click', async () => {
            ...
            const sendUrl = currentDocType === 'ND' ? API_ND_URL : API_NC_URL;
            // ND uses 'ndoc' parameter as well, structure is similar.
            ...
            resultBox.innerText = `Enviando ${currentDocType}...`;
            try {
