...
if ($action) {
    ...
    switch ($action) {
        ...
        case 'validar_cert_pass':
            ...
            // Si es error de algoritmo no soportado (legacy), intentar validación alternativa
            if (strpos($opensslError, 'unsupported') !== false || strpos($opensslError, 'legacy') !== false) {
                // Para certificados legacy, verificar estructura básica
                // Si el archivo tiene el header PKCS12 y no está vacía la contraseña, asumir ok
                // El certificado real se validará al momento de firmar
                if (strlen($certPass) >= 4 && strlen($pkcs12) > 100) {
                    echo json_encode([
                        'success' => true,
                        'message' => 'Certificado legacy detectado. La validación completa se realizará al firmar.',
                        'cert_path' => $foundPath,
                        'legacy' => true
                    ]);
                    exit;
        ...
        case 'sifen_lookup':
            try {
                ...
                $baseDir = __DIR__; // Usar directorio actual (public_html)
