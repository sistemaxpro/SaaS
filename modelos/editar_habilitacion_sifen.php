            ...
            <div class="form-section">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button type="button" @click="limpiarFormulario()" class="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition">
        ...
        function habilitacionForm() {
            return {
                ...
                extraerActividades(texto) {
                    ...
                    for (let i = 0; i < lineas.length; i++) {
                        ...
                        if (enSeccionActividades && linea.length > 0) {
                            // Patrón: línea que comienza con 6 dígitos (código de actividad)
                            ...
                            if (match) {
                                ...
                                if (descripcion.length > 3 && !descripcion.match(/^[\d\s]+$/)) {
                                    actividades.push({
                                        ...
                                        principal: actividades.length === 0 ? 1 : 0 // Primera actividad como principal
                                    });
                        ...
                        while ((match = regex.exec(texto)) !== null) {
                            ...
                            if (!codigosEncontrados.has(codigo) && descripcion.length > 3) {
                                ...
                                actividades.push({
                                    ...
                                    codigo: codigo,
                                    descripcion: descripcion.substring(0, 100),
                                    principal: actividades.length === 0 ? 1 : 0
                                });
                            }
                        }
                    }
                ...
                extraerDatosDelOCR(texto) {
                    ...
                    const rucMatch = texto.match(/(\d{8})/);
                    if (rucMatch) {
                    ...
                    // Buscar DV (Dígito Verificador - después de RUC con múltiples patrones)
                    let dv = '';
                    // Patrón 1: RUC 12345678-1 o RUC 12345678 1
                    const dvMatch1 = texto.match(/(\d{8})[\s\-:]+([0-9])(?:[^\d]|$)/);
                    if (dvMatch1) {
                        dv = dvMatch1[2];
                    } else {
                        // Patrón 2: Buscar después de "RUC" en el texto
                        const dvMatch2 = texto.match(/RUC[\s:-]*(\d{8})[\s:-]+([0-9])/);
                        if (dvMatch2) {
                            dv = dvMatch2[2];
                        }
                    }
                    ...
                    // Buscar Departamento (buscar palabras clave)
                    const dptosPatterns = ['ASUNCION', 'ALTO PARANA', 'CENTRAL', 'ITAPUA', 'CAAGUAZU', 'CORRIENTES', 'CANINDEGU', 'CAAZAPA', 'GUAIRA', 'CONCEPCION', 'SAN JUAN BAUTISTA', 'AMAMBAY', 'MISIONES', 'PARAGUARI', 'ÑEEMBUCU', 'PRESIDENTE HAYES', 'ALTO PARAGUAY', 'BOQUERON'];
                    for (let dpto of dptosPatterns) {
                        if (texto.includes(dpto)) {
                            this.formData.departamento = dpto;
                            datosExtraidos.departamento = dpto;
                            break;
                        }
                    }

                    // Buscar Dirección (buscar después de "Dirección" o patrones comunes)
                    const direccionMatch = texto.match(/DIRECCION[:\s]*([\w\s\-\.Nº#,Á-ÿ]+?)(?=\n|TELEFO|EMAIL|$)/i);
                    if (direccionMatch) {
                    ...
                    if (telefonoMatch) {
                        ...
                        datosExtraidos.telefono = telefono;
                    }
                    ...
                    // Buscar Representante Legal - CI (después de "Representante" o patrones comunes)
                    const representanteCIMatch = texto.match(/REPRESENTANTE[:\s]*([A-Z\s]+)[:\s]*(\d{5,8}[-.]?\d{1})/i);
                    if (representanteCIMatch) {
                        const ci = representanteCIMatch[2].replace(/[\s\-\.]/g, '');
                        this.formData.representante_documento = ci;
                        datosExtraidos.representante_documento = ci;
                    } else {
                        // Buscar solo números de CI (5-8 dígitos + 1 dígito de verificación)
                        const ciMatch = texto.match(/(\d{5,8})[-.]?(\d{1})(?:\s|$|[^\d])/);
                        if (ciMatch) {
                            const ci = ciMatch[1] + ciMatch[2];
                            this.formData.representante_documento = ci;
                            datosExtraidos.representante_documento = ci;
                        }
                    }
                    ...
                    // Estrategia 1: Buscar después de "REPRESENTANTE LEGAL" - excluir la palabra LEGAL del capturado
                    ...
                    if (nombreRepMatch1 && nombreRepMatch1[1]) {
                        ...
                        temp = temp.replace(/^(LEGAL|DOCUMENTO|C\.I\.|DNI|CEDULA)[\s:]*/i, '').trim();
                        if (temp && temp.length > 2 && !temp.match(/^\d+$/)) {
                            nombreRepresentante = temp;
                        }
                    }

                    // Estrategia 2: Buscar línea por línea más robustamente
                    ...
                    // Estrategia 3: Buscar patrón "REPRESENTANTE [LEGAL] [NOMBRE] [CI]"
                    if (!nombreRepresentante || nombreRepresentante.length < 3) {
                ...
                async buscarRucSifen() {
                    const ruc = prompt('Ingrese el RUC o Cédula a consultar:');
                    if (!ruc) return;
                    ...
                    try {
                        ...
                        if (data.success) {
                            ...
                            this.showToast('✓ Datos cargados de SIFEN');
                        } else {
                ...
                async validarCertPass() {
                    ...
                    try {
                        ...
                        const data = await res.json();
                        if (data.success) {
                            this.showToast('Contraseña correcta ✓');
                        } else {
                ...
                async guardarYCargarDocumentosActividades() {
                    try {
                        ...
                        if (!this.formData.ruc || !this.formData.razon_social || !this.formData.numero_timbrado) {
                            this.showToast('Complete campos: RUC, Razón Social, Timbrado', 'error');
                            return;
                        }

                        // 3. Guardar habilitación
                ...
                async guardar() {
                    ...
                    if (!this.formData.ruc || !this.formData.razon_social || !this.formData.numero_timbrado) {
                        this.showToast('Complete los campos requeridos (RUC, Razón Social, Timbrado)', 'error');
                        return;
                    }
