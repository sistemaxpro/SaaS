...
// Modo "Mi Venta": se activa desde mi_venta.php
$miVentaMode = !empty($miVentaMode);
...
$pageTitle = $miVentaMode ? t('ventas.my_title') : t('ventas.title');
?>
<!DOCTYPE html>
<html lang="<?= htmlspecialchars(SmxI18n::getLocale()) ?>" x-data="ventasApp()" x-init="init()">
<head>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .ag-theme-quartz-dark {
            ...
            --ag-popup-shadow: 0 24px 48px rgba(15, 23, 42, 0.22);
            --ag-card-shadow: 0 24px 48px rgba(15, 23, 42, 0.22);
            --ag-menu-shadow: 0 24px 48px rgba(15, 23, 42, 0.22);
        }
        .ag-theme-quartz-dark {
            ...
            --ag-popup-shadow: 0 28px 56px rgba(2, 6, 23, 0.62);
            --ag-card-shadow: 0 28px 56px rgba(2, 6, 23, 0.62);
            --ag-menu-shadow: 0 28px 56px rgba(2, 6, 23, 0.62);
        }
        .ventas-grid {
            width: 100%;
            height: 100%;
        ...
        .ag-theme-quartz .ag-column-menu,
        .ag-theme-quartz .ag-filter-menu,
        .ag-theme-quartz .ag-picker-field-wrapper,
        .ag-theme-quartz .ag-dialog,
        .ag-theme-quartz-dark .ag-menu,
        .ag-theme-quartz-dark .ag-popup-child,
        .ag-theme-quartz-dark .ag-menu-list,
        .ag-theme-quartz-dark .ag-column-menu,
        .ag-theme-quartz-dark .ag-filter-menu,
        .ag-theme-quartz-dark .ag-picker-field-wrapper,
        .ag-theme-quartz-dark .ag-dialog {
            backdrop-filter: none;
            -webkit-backdrop-filter: none;
            background: var(--ag-menu-background-color) !important;
            opacity: 1 !important;
        ...
        .ag-theme-quartz .ag-menu-option.ag-menu-option-active,
        .ag-theme-quartz-dark .ag-menu-option:hover,
        .ag-theme-quartz-dark .ag-menu-option.ag-menu-option-active {
...
<script>window.__PERMISOS__ = <?= json_encode($permisos) ?>;</script>
                ...
                <div class="flex-1 min-w-[200px]">
                    <div class="relative">
                        ...
                        <input type="text" 
                               ...
                               placeholder="<?= htmlspecialchars(t('ventas.search_placeholder')) ?>"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-200 dark:border-slate-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400"><?= htmlspecialchars(t('ventas.total_sales')) ?></p>
            ...
            <div class="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-200 dark:border-slate-700">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm text-gray-500 dark:text-gray-400"><?= htmlspecialchars(t('common.average')) ?></p>
    ...
    function ventasApp() {
        return {
            ...
            miVentaMode: <?= $miVentaMode ? 'true' : 'false' ?>,
            filterIdLogin: <?= $miVentaMode ? (int)$id_login : 0 ?>,
            filterIdCaja: <?= $miVentaMode ? (int)$id_caja_def : 0 ?>,
            gridApi: null,
            gridColumnApi: null,
            ...
            periodoActivo: 'todo',
            periodos: [
                { key: 'todo', label: '<?= htmlspecialchars(t('common.all'), ENT_QUOTES) ?>' },
            ...
            async loadVentas() {
                ...
                try {
                    ...
                    if (this.filterIdLogin > 0) params.set('id_login', this.filterIdLogin);
                    if (this.filterIdCaja > 0) params.set('id_caja', this.filterIdCaja);
                    
                    const response = await fetch(`/public/ventas/api/list.php?${params}`);
            ...
            async consultarSifen(venta) {
                try {
                    ...
                    const data = await response.json();
                    this.showToast(data.success ? 'success' : 'info', data.message || data.error || 'Consulta SIFEN ejecutada', 'SIFEN');
                    this.loadVentas();
                } catch (error) {
                    this.showToast('error', 'Error consultando SIFEN', 'Conexión');
                }
            },
            ...
            async reenviarSifen(venta) {
                ...
                try {
                    ...
                    const data = await response.json();
                    this.showToast(data.success ? 'success' : 'info', data.message || data.error || 'Reenvío ejecutado', 'SIFEN');
                    this.loadVentas();
                } catch (error) {
                    this.showToast('error', 'Error reenviando a SIFEN', 'Conexión');
                }
            },
            ...
            exportExcel() {
                ...
                window.location.href = `/public/ventas/api/export_excel.php?${params.toString()}`;
            },
