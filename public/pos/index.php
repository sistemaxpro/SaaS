...
try {
    ...
    $stmt_init = $pdo_init->prepare("SELECT empresa, fe, logos, disable_background_video FROM empresa WHERE id_empresa = :id");
    ...
    if ($emp_data) {
        ...
        $disableBackgroundVideo = (int)($emp_data['disable_background_video'] ?? 0) === 1;
    }
    ...
    if ($id_caja > 0 && $dbEmpresa) {
        try {
            ...
            $selectCols = $hasMetodoCobroPermitido ? 'impresora, metodo_cobro_permitido' : 'impresora';
            $stmt_imp = $pdo_init->prepare("SELECT $selectCols FROM $dbEmpresa.cajas WHERE id_caja = :id AND id_empresa = :emp");
            $stmt_imp->execute([':id' => $id_caja, ':emp' => $id_empresa]);
            $caja_cfg = $stmt_imp->fetch(PDO::FETCH_ASSOC) ?: [];
            if (empty(trim((string)($caja_cfg['impresora'] ?? '')))) {
                $stmt_imp2 = $pdo_init->prepare("SELECT $selectCols FROM $dbEmpresa.cajas WHERE id_caja = :id LIMIT 1");
...
?>
<!DOCTYPE html>
<html lang="es" x-data="posApp()" x-init="init()" class="dark">
<script>
...
<body class="pos-dark-solid text-gray-900 dark:text-white transition-colors">
    <!-- Video Background -->
    ...
    <div class="pos-container ios-desk-shell">
        ...
        <header class="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-2 flex items-center justify-between gap-4 rounded-t-[19px]">
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
                ...
                <span class="hidden lg:block text-white/90 font-bold text-xs uppercase tracking-widest truncate max-w-[200px]" style="text-shadow: 0 1px 10px rgba(255,255,255,.35);"><?php echo htmlspecialchars($nombreEmpresa); ?></span>
            </div>
            ...
            <div class="flex items-center gap-4 flex-shrink-0">
                ...
                <button
                    x-show="true"
                    @click="window.open('/public/misventas/index.php', '_self')"
                    class="bg-amber-600 hover:bg-amber-700 text-white font-bold p-2 rounded-lg transition-all flex items-center justify-center text-sm gap-1.5 shadow-sm"
                    title="Abrir Mis Ventas">
                ...
                <!-- Botón Cajas -->
                <?php if (mb_strtolower($rol_usuario, 'UTF-8') !== 'vendedor'): ?>
                    <button
                        x-show="true"
                        @click="window.open('/public/pos/caja.php?id_caja=<?php echo (int)$id_caja; ?>&id_empresa=<?php echo (int)$id_empresa; ?>&v=<?php echo @filemtime(__DIR__ . '/caja.php') ?: time(); ?>', '_self')"
                        class="bg-green-600 hover:bg-green-700 text-white font-bold p-2 rounded-lg transition-all flex items-center justify-center text-sm gap-1.5 shadow-sm"
                        title="Abrir Caja">
                ...
                <div class="flex items-center gap-3 bg-white dark:bg-slate-800/50 px-3 py-1.5 rounded-xl border border-slate-200 dark:border-slate-700/50 ios-desk-chip">
                    <!-- Indicador impresión local -->
                    <button
                        @click="handleAgentStatusClick()"
                        class="inline-flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-bold transition-colors cursor-pointer"
                        :class="qzConnected
                        ...
                        <span
                            class="inline-flex shrink-0 items-center justify-center w-2.5 h-2.5 rounded-full border border-white/30"
                            style="min-width:10px; min-height:10px;"
                            :class="qzConnected ? 'bg-emerald-500 shadow-[0_0_0_2px_rgba(16,185,129,.25)] animate-pulse' : 'bg-rose-500 shadow-[0_0_0_2px_rgba(244,63,94,.25)]'"></span>
                    ...
                    <button
                        ...
                        :title="directPrintEnabled
                            ? 'Impresión directa habilitada. Click para desactivar.'
                            : 'Impresión directa desactivada. Click para habilitar.'">
                        <i class="fas" :class="directPrintEnabled ? 'fa-toggle-on' : 'fa-toggle-off'"></i>
                        <span x-text="directPrintEnabled ? 'Directa ON' : 'Directa OFF'"></span>
                    </button>
                    <div class="w-px h-6 bg-slate-100 dark:bg-slate-700"></div>
                    <div class="flex flex-col items-end leading-none">
                        <span class="text-sm text-slate-500 font-bold tracking-widest">Caja</span>
                        <span class="text-sm">#<?php echo str_pad($id_caja, 2, '0', STR_PAD_LEFT); ?></span>
                    </div>
                    <div class="w-px h-6 bg-slate-100 dark:bg-slate-700"></div>
                    <div class="flex items-center gap-2">
                        <div class="hidden md:flex flex-col items-end leading-none">
                            <span class="text-sm text-slate-500 font-bold tracking-widest">Usuario</span>
        ...
        <main class="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <section class="w-full md:w-[70%] border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700 flex flex-col overflow-visible ios-desk-pane">

                <!-- Búsqueda con Autocompletado -->
                <div class="p-4 border-b border-slate-200 dark:border-slate-700 relative overflow-visible">
                    <div class="flex items-center gap-2">
                    <div class="relative flex-1" @click.away="showSuggestions = false">
                        <button
                            type="button"
                        ...
                        <input
                            ...
                            x-ref="searchInput"
                            placeholder="Buscar producto por nombre, código o filtrado por precio (<500...)"
                            class="search-ice-glass w-full rounded-lg pl-10 pr-16 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 rounded-xl">
                    ...
                    <select
                        ...
                        class="text-[10px] font-bold px-2 py-1.5 rounded-lg border border-blue-500 bg-blue-50 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer shrink-0"
                ...
                <div class="flex-1 overflow-y-auto custom-scroll p-4">
                    ...
                    <div x-show="isSearchActive() && productos.length > 0" class="pb-10">
                        <div class="cart-header-glass flex items-center justify-between mb-3 px-3 py-2">
                            ...
                            <div class="flex items-center gap-2">
                                <span class="cart-header-count text-[9px] font-bold px-2 py-0.5 rounded-full" x-text="productos.length + ' encontrados'"></span>
                                <!-- Toggle View -->
                                <div class="flex bg-white dark:bg-slate-800/50 rounded-lg p-0.5 border border-slate-200 dark:border-slate-700/50">
                                    <button
                                        ...
                                        @mousedown="startViewModePress('grid')"
                                        @mouseup="cancelViewModePress()"
                                        @mouseleave="cancelViewModePress()"
                                        @touchstart.passive="startViewModePress('grid')"
                                        @touchend="cancelViewModePress()"
                                        @touchcancel="cancelViewModePress()"
                                        ...
                                        title="Clic: Grid · Pulsación larga: Fijar">
                                    ...
                                    <button
                                        ...
                                        @mousedown="startViewModePress('list')"
                                        @mouseup="cancelViewModePress()"
                                        @mouseleave="cancelViewModePress()"
                                        @touchstart.passive="startViewModePress('list')"
                                        @touchend="cancelViewModePress()"
                                        @touchcancel="cancelViewModePress()"
                                        :class="viewMode === 'list' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:text-slate-900 dark:text-white'"
                                        class="p-1 rounded transition-all"
                                        title="Clic: Lista · Pulsación larga: Fijar">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="grid grid-cols-6 gap-2">
                            <template x-for="(producto, index) in productos" :key="producto.id">
                                <div
                                    @click="POSAudio.play('success'); addToCartFast(producto)"
                                    ...
                                    class="producto-card border rounded-xl p-1.5 cursor-pointer transition-all group relative flex flex-col items-center">
                                    <!-- Imagen del producto - ancho completo -->
                                    <div class="w-full">
                                        <div x-html="getProductImageHTML(producto, 'full', 'rounded-lg shadow-md', { enableCarousel: false, suppressImage: ultraFastPosMode && !searchImagesReady })"></div>
                                    </div>
                                    <div class="w-full px-1 mt-1">
                                        <div class="flex justify-between items-center">
                                            <span class="text-[8px] font-bold text-slate-500 font-mono bg-slate-100 dark:bg-slate-900/50 px-1 rounded" x-text="producto.codigo"></span>
                                            <div class="flex gap-0.5">
                                                <span x-show="producto.referencia" class="text-[7px] text-blue-400 font-black bg-blue-400/10 px-1 rounded" x-text="producto.referencia"></span>
                                                <span x-show="producto.has_other_stock == 1" class="text-[7px] text-orange-600 bg-orange-100 dark:bg-orange-900/30 px-0.5 rounded font-bold" title="Disponible en otras sucursales">📍</span>
                                            </div>
                                        </div>
                                        <h3 class="text-[10px] font-bold text-slate-800 dark:text-slate-200 leading-tight line-clamp-2 text-center mt-0.5" x-html="highlightMatch(producto.descripcion, searchQuery)"></h3>
                                    </div>
                                    <div class="mt-1 pt-1 border-t border-slate-200 dark:border-slate-700/20 flex justify-between items-center w-full px-1">
                                        <template x-if="parseInt(producto.controla_stock) === -1">
                                            <span class="text-[8px] px-1.5 py-0.5 rounded font-bold text-white shadow-sm bg-slate-400">Sin ctrl</span>
                                        </template>
                                        <template x-if="parseInt(producto.controla_stock) !== -1 && Number(producto.stock_pendiente || 0) !== 1">
                                            <span class="text-[8px] px-1.5 py-0.5 rounded font-bold text-white shadow-sm" :class="parseFloat(producto.stock) > 0 ? 'bg-green-600' : 'bg-red-600'" x-text="'Stock: ' + (parseFloat(producto.stock) % 1 === 0 ? parseInt(producto.stock) : producto.stock)"></span>
                                        </template>
                                        <template x-if="Number(producto.stock_pendiente || 0) === 1 && parseInt(producto.controla_stock) !== -1">
                                            <span class="text-[8px] px-1.5 py-0.5 rounded font-bold text-white shadow-sm bg-slate-500">Stock...</span>
                                        </template>
                                        <span class="text-xs font-black text-blue-600 dark:text-blue-400" x-text="Number(producto.precio_pendiente || 0) === 1 ? 'Precio...' : (formatMoney(producto.precio) + ' Gs')"></span>
                                    </div>
                                </div>
                            </template>
                        </div>
                        ...
                        <div class="space-y-1">
                            <template x-for="(producto, index) in productos" :key="producto.id">
                                <div
                                    ...
                                    class="flex items-center p-2 rounded-lg border cursor-pointer transition-colors group gap-2">
                                    <!-- Imagen del producto -->
                                    <div x-html="getProductImageHTML(producto, 40, 'shadow-sm', { enableCarousel: false, suppressImage: ultraFastPosMode && !searchImagesReady })" class="flex-shrink-0"></div>
                                    <div class="flex-1 min-w-0 pr-4">
                                        ...
                                        <div class="font-bold text-sm text-slate-800 dark:text-slate-200 truncate" x-html="highlightMatch(producto.descripcion, searchQuery)"></div>
                                    </div>
                                    <div class="text-right flex-shrink-0">
                                        ...
                                        <template x-if="parseInt(producto.controla_stock) !== -1 && Number(producto.stock_pendiente || 0) === 1">
                                            <span class="text-[9px] px-2 py-0.5 rounded font-bold text-white shadow-sm inline-block min-w-[50px] text-center bg-slate-500">Stock...</span>
                                        </template>
                                    </div>
                                </div>
                            </template>
                    ...
                    <div x-show="!isSearchActive()" class="flex flex-col h-full">
                        <div class="cart-header-glass flex items-center justify-between mb-3 px-3 py-2">
                            <h2 class="cart-header-title text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
                                Productos Frecuentes
                                <span class="text-[9px] normal-case tracking-normal font-semibold px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300"
                                      x-text="`Fuente: ${popularDebug.source} · ${popularDebug.count}`"></span>
                            </h2>
                            <div class="flex items-center gap-2">
                                <span class="cart-header-count text-[9px] font-bold px-2 py-0.5 rounded-full">Top 24</span>
                                <!-- Toggle View -->
                                <div class="flex bg-white dark:bg-slate-800/50 rounded-lg p-0.5 border border-slate-200 dark:border-slate-700/50">
                                    <button
                                        @click="setViewMode('grid')"
                        ...
                        <!-- Loading skeleton - Solo visible durante carga -->
                        ...
                        <!-- Popular Grid View -->
                        <template x-if="!loadingPopular && viewMode === 'grid'">
                        <div class="grid grid-cols-6 gap-2">
                            <template x-for="(producto, idx) in popularProducts" :key="`${producto.id}-${idx}`">
                                <div
                                    @click="POSAudio.play('success'); addToCartFast(producto)"
                                    @contextmenu.prevent="openProductDetail(producto)"
                                    class="producto-card bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/30 rounded-xl p-1.5 cursor-pointer hover:bg-blue-600/10 hover:border-blue-500/30 transition-all group relative flex flex-col items-center">
                                    <!-- Imagen del producto - ancho completo -->
                                    <div class="w-full">
                                        <div x-html="getProductImageHTML(producto, 'full', 'rounded-lg shadow-md', { enableCarousel: false })"></div>
                                    </div>
                                    <div class="w-full px-1 mt-1">
                                        <div class="flex justify-between items-center">
                                            <span class="text-[8px] font-bold text-slate-500 font-mono bg-slate-100 dark:bg-slate-900/50 px-1 rounded" x-text="producto.codigo"></span>
                                        ...
                                        <h3 class="text-[10px] font-bold text-slate-800 dark:text-slate-200 leading-tight line-clamp-2 text-center mt-0.5" x-text="producto.descripcion"></h3>
                                    </div>
                                    <div class="mt-1 pt-1 border-t border-slate-200 dark:border-slate-700/20 flex justify-between items-center w-full px-1">
                                        <template x-if="parseInt(producto.controla_stock) === -1">
                                            <span class="text-[8px] px-1.5 py-0.5 rounded font-bold text-white shadow-sm bg-slate-400">Sin ctrl</span>
                        ...
                        <!-- Popular List View -->
                        <template x-if="!loadingPopular && viewMode === 'list'">
                        <div class="space-y-1">
                            <template x-for="(producto, idx) in popularProducts" :key="`${producto.id}-${idx}`">
                                <div
                                    @click="POSAudio.play('success'); addToCartFast(producto)"
                                    @contextmenu.prevent="openProductDetail(producto)"
                                    class="flex items-center p-2 rounded-lg bg-white dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700/30 cursor-pointer hover:bg-slate-100 dark:bg-slate-700/60 transition-colors group relative overflow-hidden gap-2">
                                    <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-amber-500/50 group-hover:bg-amber-500 transition-colors"></div>
                                    <!-- Imagen del producto -->
                                    <div x-html="getProductImageHTML(producto, 40, 'ml-2 shadow-sm', { enableCarousel: false })" class="flex-shrink-0"></div>
                                    <div class="flex-1 min-w-0 pr-4">
                                        <div class="flex items-center gap-2 mb-0.5">
                                            <span class="text-[10px] bg-slate-50 dark:bg-slate-900 text-slate-400 px-1.5 py-0.5 rounded font-mono" x-text="producto.codigo"></span>
                                            <span x-show="producto.referencia" class="text-[9px] text-blue-400 font-bold" x-text="producto.referencia"></span>
                                        ...
                                        <div class="font-bold text-sm text-slate-800 dark:text-slate-200 truncate" x-text="producto.descripcion"></div>
                                    </div>
                                    <div class="text-right flex-shrink-0">
                                        <div class="font-black text-blue-600 dark:text-blue-400 text-sm" x-text="Number(producto.precio_pendiente || 0) === 1 ? 'Precio...' : formatMoney(producto.precio)"></div>
                                        <template x-if="parseInt(producto.controla_stock) === -1">
                                            <span class="text-[9px] px-2 py-0.5 rounded font-bold text-white shadow-sm inline-block min-w-[50px] text-center bg-slate-400">No controla stock</span>
                                        </template>
                                        <template x-if="parseInt(producto.controla_stock) !== -1 && Number(producto.stock_pendiente || 0) !== 1">
                        ...
                        <div x-show="!loadingPopular && popularProducts.length === 0" class="text-xs text-slate-500 dark:text-slate-400 bg-white/70 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2">
                            Sin productos frecuentes. Fuente actual: <span class="font-mono" x-text="popularDebug.source"></span>.
                        </div>
                    </div>


                    <!-- Empty Search state -->
                    <div x-show="isSearchActive() && productos.length === 0 && !loading" class="text-center text-slate-500 py-10">
            ...
            <section class="w-full md:w-[30%] h-auto md:h-full flex flex-col bg-slate-850 ios-desk-pane">

                <!-- Items del Carrito -->
                <div class="flex-1 overflow-y-auto custom-scroll max-h-[50vh] md:max-h-none px-2 py-2">
                    <!-- Header -->
                    <div class="cart-header-glass flex items-center justify-between mb-2 px-3 py-2">
                        <span class="cart-header-title text-[10px] font-black uppercase tracking-widest">Items en factura</span>
                            ...
                            <div class="cart-item group relative rounded-lg border transition-all"
                                ...
                                <div class="pl-3 pr-2 py-1.5 flex gap-2">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <!-- Línea 1: Descripción + Delete -->
                                        <div class="flex items-start gap-2">
                                            <div class="flex-1 min-w-0">
                                                <template x-if="item.editableDescripcion">
                                                    <input type="text" class="w-full text-[11px] font-semibold bg-transparent border-b border-blue-400 focus:outline-none py-0.5" :value="item.descripcion" @input="updateDescripcion(index, $event.target.value)">
                                                </template>
                                                <template x-if="!item.editableDescripcion">
                                            ...
                                            <button @click="POSAudio.play('warning'); removeFromCart(index)" 
                                        ...
                                        <!-- Línea 2: Código + Cantidad × Precio = Total -->
                ...
                <div class="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-4 rounded-br-[19px] ios-desk-footer">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative flex items-center">
                            ...
                            <!-- Dropdown sin resultados - sugerencia buscar en SET -->
                            <div
                                                ...
                                                <tr class="align-top border-b border-slate-700/70">
                                                    <td class="px-3 py-3">
                                                        <div class="flex items-center justify-between gap-2">
                                                            <input type="text" :value="group.marca_cod_conversion" list="preview-catalogo-marcas-cod-conversion" @input="previewEquivSetEquivalenciaGroupField(group, 'marca_cod_conversion', $event.target.value)" @blur="previewEquivEnsureCatalogValue('marcas_cod_conversion', $event.target.value)" class="w-full border-0 border-b border-slate-700 bg-transparent px-0 py-1 text-[13px] font-light uppercase leading-none text-[#7ae07d] focus:border-cyan-400 focus:ring-0">
                                                        ...
                                                        <div class="mt-2 space-y-1.5">
                                                            <template x-for="groupRow in group.conversionRows" :key="'left-conv-' + groupRow._idx">
                                                                <div class="flex items-center gap-1">
                                                                    <input type="text" x-model="previewEquivEditor.form.equivalentes[groupRow._idx].conversion" class="w-full border-0 border-b border-slate-700/70 bg-transparent px-0 py-1 text-[13px] font-light text-slate-100 focus:border-cyan-400 focus:ring-0">
                                                ...
                                                <div x-show="group.showAppGroup" class="bg-slate-950/40">
                                                    <div class="grid grid-cols-[1.5fr_0.8fr_1.1fr_1fr_1fr_72px] items-start">
                                                        <div class="border-r border-slate-700/80 px-3 py-3">
                                                            <div class="flex items-center justify-between gap-2">
                                                                <input type="text" :value="group.marca_aplicacion" list="preview-catalogo-marcas-aplicacion" @input="previewEquivSetAplicacionGroupField(group, 'marca_aplicacion', $event.target.value)" @blur="previewEquivEnsureCatalogValue('marcas_aplicacion', $event.target.value)" class="w-full border-0 border-b border-slate-700 bg-transparent px-0 py-1 text-[13px] font-light uppercase leading-none text-[#7ae07d] focus:border-cyan-400 focus:ring-0">
                                                        ...
                                                        <div class="col-span-5 divide-y divide-slate-800/70">
                                                            <template x-for="row in previewEquivGetAplicacionGroupRows(group)" :key="'app-row-' + row._idx">
                                                                <div class="grid grid-cols-[0.8fr_1.1fr_1fr_1fr_72px] items-center">
                                                                    <div class="border-r border-slate-800/70 px-3 py-2"><input type="text" x-model="previewEquivEditor.form.aplicaciones[row._idx].anio" list="preview-catalogo-anios-aplicacion" @blur="previewEquivEnsureCatalogValue('anios_aplicacion', $event.target.value)" class="w-full border-0 bg-transparent px-0 py-1 text-[13px] font-light text-slate-100 focus:ring-0"></div>
                                                                    <div class="border-r border-slate-800/70 px-3 py-2"><input type="text" x-model="previewEquivEditor.form.aplicaciones[row._idx].vehiculo_modelo" list="preview-catalogo-modelos-aplicacion" @blur="previewEquivEnsureCatalogValue('modelos_aplicacion', $event.target.value)" class="w-full border-0 bg-transparent px-0 py-1 text-[13px] font-light text-slate-100 focus:ring-0"></div>
                                                                    <div class="border-r border-slate-800/70 px-3 py-2"><input type="text" x-model="previewEquivEditor.form.aplicaciones[row._idx].motor" list="preview-catalogo-motores-aplicacion" @blur="previewEquivEnsureCatalogValue('motores_aplicacion', $event.target.value)" class="w-full border-0 bg-transparent px-0 py-1 text-[13px] font-light text-slate-100 focus:ring-0"></div>
                                                                    ...
                                                                    <div class="flex items-center justify-center px-2 py-2">
                                                                        <button type="button" @click="previewEquivEliminarAplicacionFila(row._idx)" class="flex h-7 w-7 items-center justify-center rounded-full border border-slate-600 text-red-300 hover:bg-slate-800"><i class="fas fa-trash text-[10px]"></i></button>
                                                                    </div>
                                                                </div>
                                                            </template>
                                                        </div>
        ...
        <datalist id="preview-catalogo-marcas-cod-conversion"><template x-for="item in previewEquivEditor.catMarcasCodConversion" :key="'eq-catalog-cod-' + item.id"><option :value="item.nombre"></option></template></datalist>
        <datalist id="preview-catalogo-marcas-aplicacion"><template x-for="item in previewEquivEditor.catMarcasAplicacion" :key="'eq-catalog-app-' + item.id"><option :value="item.nombre"></option></template></datalist>
        <datalist id="preview-catalogo-anios-aplicacion"><template x-for="item in previewEquivEditor.catAniosAplicacion" :key="'eq-catalog-anio-' + item.id"><option :value="item.nombre"></option></template></datalist>
        <datalist id="preview-catalogo-modelos-aplicacion"><template x-for="item in previewEquivEditor.catModelosAplicacion" :key="'eq-catalog-modelo-' + item.id"><option :value="item.nombre"></option></template></datalist>
        <datalist id="preview-catalogo-motores-aplicacion"><template x-for="item in previewEquivEditor.catMotoresAplicacion" :key="'eq-catalog-motor-' + item.id"><option :value="item.nombre"></option></template></datalist>
        <datalist id="preview-catalogo-codigos-motor-aplicacion"><template x-for="item in previewEquivEditor.catCodigosMotorAplicacion" :key="'eq-catalog-codmotor-' + item.id"><option :value="item.nombre"></option></template></datalist>
        ...
        <div
            ...
            <div class="relative flex flex-col items-center gap-4 max-w-full">
                <div
                    ...
                    <div class="flex-1 overflow-y-auto bg-white">
                        <template x-if="showTicketModal">
                            <iframe id="webPrintFrame" :src="ticketUrl" class="w-full h-full min-h-[70vh] border-none"></iframe>
                        </template>
                    </div>
                </div>
                ...
                <div x-show="!webPrintModalMode" class="flex gap-4 w-full justify-center flex-wrap">
                    <button
                        @click="if(lastVenta) directPrint(lastVenta.id_factura, lastVenta.tipo_documento)"
                ...
                <div x-show="webPrintModalMode" class="flex gap-3 w-full justify-center flex-wrap">
                    <button
        ...
        <div
            ...
            <div
                ...
                class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 max-w-4xl w-full max-h-[90vh] flex flex-col overflow-hidden">
                <div class="bg-indigo-600 dark:bg-indigo-700 p-4 flex items-center justify-between">
                    <div>
                        <h3 class="text-white font-black text-lg">Importar Presupuesto</h3>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="bg-slate-50 dark:bg-slate-900 rounded-xl p-4">
                                <div class="flex items-center justify-center">
                                    <div x-html="getProductImageHTML(getDetailImageProduct(), 220, 'rounded-xl shadow-md', { enableCarousel: false })"></div>
                                </div>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <div class="bg-slate-50 dark:bg-slate-900 rounded-xl p-4">
                                <div class="mb-3 flex items-center justify-between gap-3">
                                    ...
                                    <button @click="openPreviewEquivalentesEditorFromDetail()" type="button" class="rounded-lg border border-slate-300 bg-white px-3 py-1.5 text-[13px] font-light text-slate-700 transition hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">
                                        Ver / editar
                                    </button>
                                </div>
            ...
            <div
                ...
                <div class="bg-gradient-to-r from-blue-700 to-cyan-700 px-6 py-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-white/80" x-text="(kardexProducto?.descripcion || '') + (kardexSucursal?.nombre ? (' • ' + kardexSucursal.nombre) : '')"></p>
                    </div>
                ...
                <div class="flex-1 overflow-auto p-4">
                    ...
                    <div x-show="!loadingKardex">
                            ...
                            <table class="w-full text-sm">
                                <thead class="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300">
                                    <tr>
                                        ...
                                        <th class="px-3 py-2 text-right">Usuario</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                                    <template x-for="mov in kardexMovimientos" :key="(mov.id_mov || 0) + '_' + (mov.fecha || '')">
                                        <tr>
                                            ...
                                            <td class="px-3 py-2 text-right text-slate-700 dark:text-slate-200" x-text="mov.idfactura || '-'"></td>
                                            <td class="px-3 py-2 text-right text-slate-500 dark:text-slate-300" x-text="mov.id_login || '-'"></td>
                                        </tr>
                                    </template>
                                </tbody>
                            </table>
                        ...
                        <div x-show="kardexMovimientos.length === 0" class="text-center py-10 text-slate-500">
                            Sin movimientos para este producto en la sucursal seleccionada.
                        </div>
                    </div>
                </div>
        ...
        <div
            ...
            x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0"
            @keydown.escape.window="showImageSearchModal = false; imageUrlInput = ''; imageBase64 = ''; searchResults = []; searchTranslated = ''"
            ...
            <div
                ...
                x-transition:enter-start="opacity-0 scale-95"
                x-transition:enter-end="opacity-100 scale-100"
                @click.away="showImageSearchModal = false; imageUrlInput = ''; imageBase64 = ''; searchResults = []; searchTranslated = ''"
                class="bg-white dark:bg-slate-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
                ...
                <div class="bg-gradient-to-r from-blue-600 to-cyan-600 px-6 py-4 flex items-center justify-between">
                    ...
                    <button @click="showImageSearchModal = false; imageUrlInput = ''; imageBase64 = ''; searchResults = []; searchTranslated = ''" class="text-white/80 hover:text-white text-2xl">&times;</button>
                </div>

                <!-- Contenido -->
                <div class="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
                    <!-- Búsqueda automática en Pixabay -->
                    <div class="bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-xl p-4 border border-purple-200 dark:border-purple-800">
                        <div class="flex items-center gap-2 mb-3">
                            <span class="w-6 h-6 bg-purple-600 rounded-full flex items-center justify-center text-white text-xs font-bold">1</span>
                            <span class="font-semibold text-slate-700 dark:text-slate-300">Búsqueda automática</span>
                            <span class="text-xs bg-purple-100 dark:bg-purple-800 text-purple-700 dark:text-purple-300 px-2 py-0.5 rounded-full">Pixabay</span>
                        </div>
                        
                        <button 
                            @click="searchImagesPixabay()"
                            :disabled="searchingImages"
                            class="w-full bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white font-semibold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-50">
                            <template x-if="searchingImages">
                                <i class="fas fa-spinner fa-spin"></i>
                            </template>
                            <template x-if="!searchingImages">
                                <i class="fas fa-search"></i>
                            </template>
                            <span x-text="searchingImages ? 'Buscando...' : 'Buscar imágenes'"></span>
                        </button>
                        
                        <!-- Info de traducción -->
                        <div x-show="searchTranslated && searchTranslated !== imageSearchProduct?.descripcion" class="mt-2 text-xs text-purple-600 dark:text-purple-400">
                            <i class="fas fa-language"></i> Traducido: "<span x-text="searchTranslated"></span>"
                        </div>
                        
                        <!-- Resultados de búsqueda -->
                        <div x-show="searchResults.length > 0" class="mt-4">
                            <p class="text-xs text-slate-500 mb-2">
                                <span x-text="searchTotal"></span> resultados encontrados - Clic en una imagen para seleccionar
                            </p>
                            <div class="grid grid-cols-4 gap-2 max-h-48 overflow-y-auto">
                                <template x-for="img in searchResults" :key="img.id">
                                    <div 
                                        @click="selectPixabayImage(img.preview)"
                                        class="relative aspect-square bg-slate-200 dark:bg-slate-700 rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-purple-500 transition-all group">
                                        <img :src="img.thumbnail" 
                                             class="w-full h-full object-cover"
                                             loading="lazy">
                                        <div class="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center">
                                            <i class="fas fa-check-circle text-white text-xl opacity-0 group-hover:opacity-100 transition-all"></i>
                                        </div>
                                    </div>
                                </template>
                            </div>
                            <!-- Botón cargar más -->
                            <button 
                                x-show="searchResults.length < searchTotal"
                                @click="loadMoreImages()"
                                :disabled="searchingImages"
                                class="w-full mt-2 py-2 text-sm text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-purple-900/30 rounded-lg transition">
                                <i class="fas fa-plus"></i> Cargar más imágenes
                            </button>
                        </div>
                        
                        <!-- Sin resultados -->
                        <div x-show="!searchingImages && searchTranslated && searchResults.length === 0" class="mt-3 text-center text-slate-500 text-sm">
                            <i class="fas fa-image text-2xl mb-2 opacity-50"></i>
                            <p>No se encontraron imágenes</p>
                        </div>
                    </div>

                    <!-- Alternativa: Buscar en Google -->
                    <div class="bg-slate-50 dark:bg-slate-900 rounded-xl p-4">
                        <div class="flex items-center gap-2 mb-3">
                            <span class="w-6 h-6 bg-blue-600 rounded-full flex items-center justify-center text-white text-xs font-bold">2</span>
                            <span class="font-semibold text-slate-700 dark:text-slate-300">O buscar en Google</span>
                        </div>
                    ...
                    <!-- Botones -->
                    <div class="flex gap-3 pt-3 sticky bottom-0 bg-slate-50 dark:bg-slate-900 border-t border-slate-200 dark:border-slate-700 pb-2">
                        <button
                            @click="showImageSearchModal = false; imageUrlInput = ''; imageBase64 = ''; searchResults = []; searchTranslated = ''"
                            class="flex-1 px-4 py-3 bg-slate-200 text-slate-800 dark:bg-slate-700 dark:text-slate-100 hover:bg-slate-300 dark:hover:bg-slate-600 rounded-xl transition font-medium">
                            Cancelar
                        </button>
                        <button
                            ...
                            class="flex-1 px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl transition font-semibold flex items-center justify-center gap-2">
        ...
        <div
            ...
            <div
                ...
                <div class="p-6">
                    <div class="flex items-center gap-4">
                        <template x-if="infoModal.loading">
                            <div class="animate-spin rounded-full h-8 w-8 border-4 border-blue-600 border-t-transparent"></div>
                        </template>
                        <template x-if="!infoModal.loading">
                            <span class="text-3xl" x-text="infoModal.icon"></span>
                        </template>
                        <div class="flex-1">
                ...
                <template x-if="!infoModal.loading && infoModal.type !== 'loading'">
                    <div class="bg-slate-50 dark:bg-slate-900/50 px-6 py-3 flex justify-end">
                        <button
            ...
            <div
                ...
                <div class="p-4 overflow-y-auto custom-scroll flex-1">
                    <div class="space-y-6 max-w-xl mx-auto">
                        ...
                        <div class="bg-slate-50 dark:bg-slate-900/50 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-700 p-4 space-y-4">
                        ...
                        <div x-show="selectedPaymentMethod" class="bg-blue-600/5 dark:bg-blue-600/10 rounded-2xl border border-blue-500/30 p-4 space-y-4 animate-in zoom-in-95 duration-200">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <!-- PIX -->
                                    <template x-if="selectedPaymentMethod === 'pix'">
                                        <div>
                        ...
                        <div x-show="remainingAmount > 0 && !selectedPaymentMethod && !userConfig.isVendedor" class="animate-in fade-in slide-in-from-top-4 duration-300">
        ...
        <script>
            ...
            function posApp() {
                return {
                    ...
                    searchProductsCacheTtlMs: 0, // DEV: cache desactivada (prod: 2 * 60 * 1000)
                    ...
                    init() {
                        ...
                        // Refrescar impresora de caja al abrir (evita estado vacío en POS)
                        this.loadPrintConfig();

                        // Detectar país del navegador para WhatsApp
                    ...
                    isNotaComunDocType(docType) {
                        ...
                        return !(dt === '3' || dt === 'electro' || dt === '1' || dt === 'auto');
                    },
                    ...
                    openWebPrintTicket(idFactura, docType, nroFactura = '') {
                        const endpoint = 'ticket.php';
                        const nroParam = encodeURIComponent((nroFactura || '').trim());
                        const url = `${endpoint}?id=${idFactura}&id_empresa=${this.idEmpresa}&width=48&nro=${nroParam}&logo=0&pdf=1&in_modal=1`;
                        this.ticketUrl = url;
                    ...
                    async directPrint(idFactura, docType, nroFactura = '', options = {}) {
                        ...
                        if (!this.directPrintEnabled) {
                            this.openWebPrintTicket(idFactura, tipoDoc, nroFactura);
                            return;
                        }
                        ...
                        try {
                            ...
                            const res = await fetch(`${endpoint}?id=${idFactura}&id_empresa=${this.idEmpresa}&width=48&nro=${nroParam}&logo=0&qr_mode=${encodeURIComponent(qrMode)}`);
                    ...
                    canUseLocalAgent() {
                        if (this.isAndroidNativePrintBridge()) return true;
                    ...
                    async launchNativeAgentFromWeb() {
                        ...
                        try {
                            if (this.isAndroidNativePrintBridge()) {
                                ...
                                await this.showAlert(
                                    'Impresión Android',
                                    'SistemaX Pro ya integra impresión Bluetooth. Si no conecta, revisá permisos Bluetooth y que la impresora esté emparejada en Android.',
                                    '🖨️',
                                    'warning'
                                );
                                return;
                    ...
                    async searchProducts() {
                        // No buscar si el primer carácter es ".", "+" o "*" (atajos POS).
                        ...
                        try {
                            ...
                            this.writeSearchProductsCache(cacheKey, productos);
                            if (this.ultraFastPosMode) {
                                this.scheduleSearchResultHydration(requestId, {
                                    q: searchTerms,
                                    category: this.filterCategory || '',
                                    estado: this.searchEstado,
                                    tipo_precio: this.selectedPriceType,
                    ...
                    async handleSearchEnter() {
                        ...
                        if (this.productos.length > 0) {
                            await this.addToCartFast(this.productos[this.selectedIndex]);
                            this.searchQuery = '';
                            this.productos = [];
                            this.playSound('success');
                            return;
                        ...
                        if (found) {
                            await this.addToCartFast(found);
                            this.showSuggestions = false;
                            this.playSound('success');
                            return;
                        }
                        ...
                        try {
                            ...
                            if (exact) {
                                await this.addToCartFast(exact);
                                this.showSuggestions = false;
                                this.playSound('success');
                            } else if (results.length === 1) {
                                await this.addToCartFast(results[0]);
                                this.showSuggestions = false;
                                this.playSound('success');
                            } else {
                                this.showInfoModal({
                    ...
                    async selectSuggestion(index) {
                        if (this.productos.length > 0 && index >= 0 && index < this.productos.length) {
                            await this.addToCartFast(this.productos[index]);
                            this.showSuggestions = false;
                        }
                    },
                    ...
                    async confirmSimpleSale() {
                        ...
                        try {
                            ...
                            if (result.success) {
                                ...
                                await this.markImportedPresupuestoAsConverted(idFactura);
                    ...
                    async markImportedPresupuestoAsConverted(ventaId) {
                        const imported = this.importedPresupuesto;
                        const presupuestoId = parseInt(imported?.id_factura || 0, 10);
                        const facturaId = parseInt(ventaId || 0, 10);
                        if (presupuestoId <= 0 || facturaId <= 0) return;
                        try {
                            const res = await fetch('api/presupuestos_manage.php?action=convert', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({
                                    id_factura: presupuestoId,
                                    converted_factura_id: facturaId,
                                    id_empresa: this.idEmpresa
                                })
                            });
                            const data = await res.json();
                            if (!data.success) {
                                console.warn('No se pudo marcar presupuesto como convertido:', data.message || data.error || data);
                            }
                        } catch (e) {
                            console.warn('Error marcando presupuesto convertido:', e);
                        }
                    },
                    ...
                    async updateVenta() {
                        ...
                        try {
                            ...
                            if (result.success) {
                                ...
                                this.clearCart();
                                this.toast('Venta actualizada e impresa', 'success');
                                this.playSound('success');
                            } else {
                    ...
                    printTicket() {
                        if (this.lastVenta?.id_factura) {
                            this.directPrint(this.lastVenta.id_factura, this.lastVenta.tipo_documento);
                        }
                    },
                    ...
                    previewEquivNormalizeEquivalencias(rows) {
                        return (Array.isArray(rows) ? rows : [])
                            .map((row, idx) => ({ marca_cod_conversion: String(row?.marca_cod_conversion || '').trim(), conversion: String(row?.conversion || '').trim(), orden: idx + 1 }))
                    ...
                    previewEquivNormalizeAplicaciones(rows) {
                        return (Array.isArray(rows) ? rows : [])
                            ...
                            .filter((row) => row.marca_aplicacion !== '' || row.vehiculo_marca !== '' || row.vehiculo_modelo !== '' || row.anio !== '' || row.motor !== '' || row.codigo_motor !== '');
                    },

                    previewEquivAplicacionTieneDetalle(row) {
                        if (row?.__is_app_group) return false;
                        if (row?.__is_new_app) return true;
                        return [row?.vehiculo_marca, row?.vehiculo_modelo, row?.anio, row?.motor, row?.codigo_motor].some((value) => String(value || '').trim() !== '');
                    },
                    ...
                    previewEquivEquivalenciasAgrupadas() {
                        ...
                        (Array.isArray(this.previewEquivEditor.form?.equivalentes) ? this.previewEquivEditor.form.equivalentes : []).forEach((row, idx) => {
                            ...
                            if (!groups.has(key)) groups.set(key, { key, marca_cod_conversion: marcaCod, indexes: [], conversionRows: [] });
                    ...
                    previewEquivAplicacionesAgrupadas() {
                        ...
                        (Array.isArray(this.previewEquivEditor.form?.aplicaciones) ? this.previewEquivEditor.form.aplicaciones : []).forEach((row, idx) => {
                            ...
                            if (!groups.has(key)) {
                                groups.set(key, { key, group_token: groupToken, marca_aplicacion: marcaAplic, indexes: [], appRows: [], hasAppGroupHeader: false, showAppGroup: false });
                            }
                            const group = groups.get(key);
                            group.indexes.push(idx);
                            if (row?.__is_app_group) group.hasAppGroupHeader = true;
                            else if (this.previewEquivAplicacionTieneDetalle(row)) group.appRows.push({ ...row, _idx: idx });
                        });
                        groups.forEach((group) => {
                            const resumen = group.appRows.map((row) => [String(row.vehiculo_marca || '').trim(), String(row.vehiculo_modelo || '').trim(), String(row.anio || '').trim(), String(row.motor || '').trim(), String(row.codigo_motor || '').trim()].filter(Boolean).join(' / ')).filter(Boolean);
                    ...
                    previewEquivGetAplicacionGroupRows(group) {
                        ...
                        return indexes.map((idx) => Number(idx)).filter((idx) => Number.isInteger(idx) && idx >= 0 && idx < this.previewEquivEditor.form.aplicaciones.length).map((idx) => ({ ...(this.previewEquivEditor.form.aplicaciones[idx] || {}), _idx: idx })).filter((row) => !row?.__is_app_group && this.previewEquivAplicacionTieneDetalle(row));
                    },

                    previewEquivSetEquivalenciaGroupField(group, field, value) {
                        const val = String(value || '');
                        (Array.isArray(group?.indexes) ? group.indexes : []).forEach((idx) => {
                            if (this.previewEquivEditor.form.equivalentes[idx]) this.previewEquivEditor.form.equivalentes[idx][field] = val;
                        });
                    },
                    ...
                    previewEquivAgregarCodigoConversion() { this.previewEquivEditor.form.equivalentes.push({ marca_cod_conversion: '', conversion: '' }); },
                    previewEquivAgregarCodigoConversionEnGrupo(group) { this.previewEquivEditor.form.equivalentes.push({ marca_cod_conversion: String(group?.marca_cod_conversion || '').trim(), conversion: '' }); },
                    previewEquivEliminarEquivalenciaFila(idx) { const pos = Number(idx); if (Number.isInteger(pos) && pos >= 0 && pos < this.previewEquivEditor.form.equivalentes.length) this.previewEquivEditor.form.equivalentes.splice(pos, 1); },
                    ...
                    previewEquivAgregarAplicacion() {
                        ...
                        this.previewEquivEditor.form.aplicaciones.push({ conversion: '', marca_aplicacion: '', vehiculo_marca: '', vehiculo_modelo: '', anio: '', motor: '', codigo_motor: '', __is_app_group: true, __group_token: groupToken });
                        this.previewEquivEditor.form.aplicaciones.push({ marca_aplicacion: '', vehiculo_marca: '', vehiculo_modelo: '', anio: '', motor: '', codigo_motor: '', __is_new_app: true, __is_app_group: false, __group_token: groupToken });
                    },

                    previewEquivAgregarAplicacionEnGrupo(group) {
                        const insertAt = Array.isArray(group?.indexes) && group.indexes.length > 0 ? (Math.max(...group.indexes.map((idx) => Number(idx)).filter((idx) => Number.isInteger(idx))) + 1) : this.previewEquivEditor.form.aplicaciones.length;
                        this.previewEquivEditor.form.aplicaciones.splice(insertAt, 0, { marca_aplicacion: String(group?.marca_aplicacion || '').trim(), vehiculo_marca: '', vehiculo_modelo: '', anio: '', motor: '', codigo_motor: '', __is_new_app: true, __is_app_group: false, __group_token: String(group?.group_token || '') });
                    },

                    previewEquivEliminarAplicacionFila(idx) { const pos = Number(idx); if (Number.isInteger(pos) && pos >= 0 && pos < this.previewEquivEditor.form.aplicaciones.length) this.previewEquivEditor.form.aplicaciones.splice(pos, 1); },
                    ...
                    async previewEquivEnsureCatalogValue(tabla, value) {
                        ...
                        if (exists) return;
                        try {
                            const res = await fetch('/public/productos/api/catalogos.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'create', tabla, nombre, id_empresa: this.idEmpresa }) });
                            const data = await res.json();
                            if (data?.success) this.previewEquivSetCatalogArray(tabla, this.previewEquivNormalizeCatalogItems(tabla, data.data || []));
                        } catch (_) {}
                    },

                    async openPreviewEquivalentesEditorFromDetail() {
                        const productId = Number(this.productDetail?.producto?.idproducto || this.productDetail?.producto?.id || 0);
                        ...
                        if (Number(this.previewEquivEditor.loadedProductId || 0) === productId && this.previewEquivEditor.form?.idproducto) return;
                    ...
                    async loadPreviewEquivEditorProduct(productId) {
                        ...
                        const data = await res.json();
                        if (!data?.success || !data?.data?.producto) throw new Error(data?.error || 'No se pudo cargar el producto');
                        ...
                        this.previewEquivEditor.form = {
                            ...
                            precios: (data.data.precios || []).map((pr) => ({ tipo: String(pr?.tipo ?? ''), tipo_nombre: String(pr?.tipo_nombre || '').trim(), costo: parseFloat(pr.costo) || 0, porcentaje: parseFloat(pr.porcentaje) || 0, precio: parseFloat(pr.precio) || 0, moneda: pr.moneda || 'PYG' })),
                            codigos_barra_extra: codigosExtra,
                            equivalentes: this.previewEquivNormalizeEquivalencias(((data.data.equivalentes || []).length ? data.data.equivalentes : (data.data.aplicaciones || [])).filter((aplic) => String(aplic?.marca_cod_conversion || '').trim() !== '' || String(aplic?.conversion || '').trim() !== '')),
                            aplicaciones: ((data.data.aplicaciones_detalle || []).length ? data.data.aplicaciones_detalle : (data.data.aplicaciones || []))
                                .filter((aplic) => String(aplic?.marca_aplicacion || '').trim() !== '' || String(aplic?.vehiculo_marca || '').trim() !== '' || String(aplic?.vehiculo_modelo || '').trim() !== '' || String(aplic?.anio || '').trim() !== '' || String(aplic?.motor || '').trim() !== '' || String(aplic?.codigo_motor || '').trim() !== '')
                    ...
                    previewEquivBuildAplicacionesPayload() {
                        return this.previewEquivNormalizeAplicaciones(this.previewEquivEditor.form.aplicaciones).map((row, idx) => ({ marca_aplicacion: row.marca_aplicacion, vehiculo_marca: row.vehiculo_marca, vehiculo_modelo: row.vehiculo_modelo, anio: row.anio, motor: row.motor, codigo_motor: row.codigo_motor, orden: idx + 1 }));
                    },
                    ...
                    async savePreviewEquivEditor() {
                        ...
                        this.previewEquivEditor.error = '';
                        try {
                            const response = await fetch('/public/productos/api/guardar.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(this.previewEquivBuildPayload()) });
                            const result = await response.json();
                            if (!result?.success) {
                    ...
                    async openKardexModal(sucursal) {
                        const idProducto = this.productDetail?.producto?.idproducto || this.productDetail?.producto?.id;
                        if (!idProducto || !sucursal?.id_sucursal) return;
                        const params = new URLSearchParams({
                            id: String(idProducto),
                            id_sucursal: String(sucursal.id_sucursal),
                            id_empresa: String(this.idEmpresa)
                        });
                        const url = `/public/pos/kardex.php?${params.toString()}`;
                    ...
                    async addToCartFromDetail() {
                        ...
                        const producto = {
                            ...
                            precio_min: p.precio_compra || 0,
                            edita_precio: p.edita_precio || 0,
                            editable: p.editable || 0,
                            usaserial: (typeof p.usaserial === 'undefined' ? null : p.usaserial)
                        };
                    ...
                    async saveProductImage() {
                        ...
                        try {
                            ...
                            if (this.imageBase64) {
                                ...
                                response = await fetch('/public/pos/api/imagen_proxy.php', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({
                                        action: 'upload_url',
                            ...
                            if (data.success) {
                                ...
                                if (this.imageBase64) {
                                    ...
                                    syncProductImageFromCache(productId, data.cache_url || '', this);
                                    this.toast('Imagen cacheada correctamente', 'success');
                                }
                                POSAudio.play('success');
            ...
            <div class="px-6 py-5 space-y-4">
                ...
                <div class="rounded-lg p-3" :class="qzConnected ? 'bg-emerald-500/10 border border-emerald-500/30' : 'bg-red-500/10 border border-red-500/30'">
                    ...
                    <div class="mt-2 ml-5 space-y-1">
                        ...
                        <template x-if="qzConnected && _qzPrintersList.length">
                            <p class="text-[10px] text-emerald-400/70">🖨️ Impresoras: <span x-text="_qzPrintersList.join(', ')"></span></p>
                        </template>
                        <template x-if="_qzLastError">
                            <p class="text-[10px] text-red-400">❌ <span x-text="_qzLastError"></span></p>
                        </template>
                    </div>
                </div>
                ...
                <div x-show="!qzConnected" class="space-y-3">
                    ...
                    <button @click="launchNativeAgentFromWeb()"
                        ...
                        <template x-if="!launchingAgent">
                            <span class="flex items-center gap-2"><i class="fas fa-rocket"></i> Iniciar Agent desde navegador</span>
                        </template>
                        <template x-if="launchingAgent">
                            <span class="flex items-center gap-2"><i class="fas fa-spinner fa-spin"></i> Iniciando Agent...</span>
                        </template>
                    </button>
                    ...
                    <button @click="retryQzConnect()"
                        ...
                        <template x-if="!qzCheckingConnection">
                            <span class="flex items-center gap-2">Reintentar conexión directa</span>
                        </template>
                        <template x-if="qzCheckingConnection">
                            <span class="flex items-center gap-2"><i class="fas fa-spinner fa-spin"></i> Conectando...</span>
                        </template>
                    </button>
