...
?>
<body class="min-h-screen bg-slate-100 text-slate-900" x-data="ticketLanding(<?= htmlspecialchars(json_encode($ticketData, JSON_UNESCAPED_UNICODE), ENT_QUOTES, 'UTF-8') ?>)">
    <div class="fixed inset-0 -z-10 overflow-hidden">
        <div class="absolute -top-24 -left-24 h-80 w-80 rounded-full bg-emerald-200/40 blur-3xl"></div>
        <div class="absolute right-0 top-24 h-96 w-96 rounded-full bg-cyan-200/40 blur-3xl"></div>
    </div>
    ...
    <main class="mx-auto flex w-full max-w-[1800px] flex-col gap-6 p-0 md:p-6 xl:p-8">
        <div class="grid grid-cols-1 xl:grid-cols-[minmax(0,3fr)_minmax(280px,1fr)] gap-0 xl:gap-6 items-start">
            <div class="space-y-6 order-2 xl:order-1">
                <section class="no-print rounded-none md:rounded-3xl border-0 md:border border-white/60 bg-white/70 p-6 shadow-none md:shadow-sm backdrop-blur-xl md:p-10">
                    ...
                    <p x-cloak x-show="copied" class="mt-3 text-xs font-medium text-emerald-700">Enlace copiado al portapapeles.</p>
                </section>

                <section class="grid gap-6 lg:grid-cols-2">
                    <article class="rounded-none md:rounded-3xl border-0 md:border border-slate-200 bg-white p-6 shadow-none md:shadow-sm md:p-8">
                        <h2 class="text-sm font-semibold uppercase tracking-[0.12em] text-slate-500">Resumen</h2>
                        <dl class="mt-5 space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <dt class="text-sm text-slate-500">Nro. de documento</dt>
                        <dd class="text-sm font-semibold text-slate-800" x-text="ticket.id"></dd>
                    </div>
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <dt class="text-sm text-slate-500">Empresa</dt>
                        <dd class="text-sm font-semibold text-slate-800" x-text="ticket.empresa"></dd>
                    </div>
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <dt class="text-sm text-slate-500">Cliente</dt>
                        <dd class="text-sm font-semibold text-slate-800" x-text="ticket.cliente"></dd>
                    </div>
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <dt class="text-sm text-slate-500">Fecha</dt>
                        <dd class="text-sm font-semibold text-slate-800" x-text="ticket.fecha"></dd>
                    </div>
                    <div x-show="ticket.empresaTelefono" class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <dt class="text-sm text-slate-500">Teléfono</dt>
                        <dd class="text-sm font-semibold text-slate-800" x-text="ticket.empresaTelefono"></dd>
                    </div>
                    <div x-show="ticket.empresaEmail" class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <dt class="text-sm text-slate-500">Email</dt>
                        <dd class="text-sm font-semibold text-slate-800" x-text="ticket.empresaEmail"></dd>
                    </div>
                    <div x-show="ticket.empresaDireccion" class="flex items-center justify-between border-b border-slate-100 pb-3">
                        <dt class="text-sm text-slate-500">Dirección</dt>
                        <dd class="text-right text-sm font-semibold text-slate-800" x-text="ticket.empresaDireccion"></dd>
                    </div>
                    <div class="flex items-center justify-between">
                        <dt class="text-sm text-slate-500">Total</dt>
                        <dd class="text-lg font-semibold text-slate-900" x-text="formatGs(ticket.total)"></dd>
                    </div>
                        </dl>
                    </article>

                    <article class="ticket-print rounded-none md:rounded-3xl border-0 md:border border-slate-200 bg-white p-6 shadow-none md:shadow-sm md:p-8">
                        <div class="mb-4 flex items-center justify-between">
                            <h2 class="text-sm font-semibold uppercase tracking-[0.12em] text-slate-500">Preview ticket</h2>
                            <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">Venta</span>
                        </div>

                        <div class="mx-auto max-w-sm rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 font-mono text-xs leading-5 text-slate-700">
                            <div class="text-center">
                                <p class="text-sm font-bold tracking-widest text-slate-900">SISTEMAX PRO</p>
                            ...
                            <div class="space-y-1">
                                ...
                                <p class="truncate text-center font-semibold text-slate-800" x-text="ticket.empresaNombre"></p>
                            ...
                            <p class="mt-2 flex justify-between text-sm font-bold text-slate-900">
                                <span>TOTAL GS</span>
                            ...
                            <div class="flex justify-center">
                                <img :src="ticket.qrUrl" alt="QR de consulta" class="h-24 w-24 rounded border border-slate-200 bg-white p-1">
                            </div>
