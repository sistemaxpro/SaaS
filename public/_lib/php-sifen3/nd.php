<?php 
	/**
	 * ND.PHP - Emisión de Nota de Débito Electrónica
	...
	if (empty($input)) {
		echo json_encode(['success' => false, 'message' => 'No se recibieron datos en el endpoint ND']);
		exit;
	}
	...
	try {
		...
		$sifen->setCSC($input['csc'] ?? 'a911929881f35219c7610A4f8F9F97D5');
		...
		$factura = new \sifen\Factura(\sifen\DTE::ND, $facturaData);
		...
		error_log("ND Lote XML Request: " . $xmlRequest->asXML());
		...
		if ($respuesta['status'] == 'ok' && !empty($respuesta['response'])) {
			...
			$protocoloAuth = null; // Initialize to null
		...
		echo json_encode([
			'success' => false,
			'message' => 'Excepción en ND.php: ' . $e->getMessage()
		]);
	}
?>