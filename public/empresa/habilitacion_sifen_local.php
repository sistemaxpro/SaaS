...
// Forzar empresa logueada (no permitir override por GET)
$id_empresa = (int)Session::getIdEmpresa();
if ($id_empresa <= 0) {
    http_response_code(403);
    die('Empresa no definida en sesión');
}

// Configuración master del proyecto
...
// Cargar solo empresa logueada (evitar edición cruzada)
$stmtEmpresas = $pdo->prepare("SELECT id_empresa, empresa, ruc, dv, timbrado FROM {$masterDb}.empresa WHERE activo = 1 AND id_empresa = ? ORDER BY empresa");
$stmtEmpresas->execute([$id_empresa]);
...
<body class="min-h-screen bg-slate-100 dark:bg-slate-900 text-slate-900 dark:text-slate-100"
        ...
        <div class="page-header">
            <div class="flex items-center gap-4">
                <button
                    @click="volver()"
                    type="button"
                    class="inline-flex items-center justify-center w-11 h-11 rounded-xl border border-red-200/70 bg-red-50 text-red-600 hover:bg-red-100 hover:text-red-700 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-400 dark:hover:bg-red-900/40 transition"
                    title="Salir">
                    ...
                    <table>
                        ...
                        <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center;">
                                            <template x-for="caja in (doc.cajas || [])" :key="`${doc.id}-${caja}`">
                                                <span class="badge badge-info" style="display:inline-flex;align-items:center;gap:6px;">
                                                    <span x-text="`Caja ${caja}`"></span>
                                                    <button type="button" @click="quitarCajaDocumento(doc, caja)" style="background:none;border:none;color:inherit;cursor:pointer;line-height:1;padding:0;" title="Quitar caja">x</button>
                                                </span>
                                            </template>
                                            <template x-if="cajasFiltradasPorDocumento(doc).length > 0">
                                                <div style="display:inline-flex;gap:6px;align-items:center;">
                                                    <select x-model="docCajaSelect[doc.id]" multiple size="3" style="min-width:180px;padding:4px 6px;border:1px solid #334155;border-radius:6px;background:transparent;">
                                                        <template x-for="c in cajasFiltradasPorDocumento(doc)" :key="c.id">
                                                            <option :value="c.id" x-text="c.label"></option>
                                                        </template>
                                                    </select>
                                                    <button type="button" class="btn btn-success btn-sm" @click="agregarCajasDocumento(doc)" title="Agregar cajas seleccionadas">
                                                        <i class="fas fa-plus"></i>
                                                    </button>
                                                </div>
                                            </template>
                                            <template x-if="cajasFiltradasPorDocumento(doc).length === 0">
                                                <span class="badge badge-warning">Sin cajas para este Est/Pto</span>
                                            </template>
                                        </div>
                                    </td>
                                    <td>
                            ...
                            <tr x-show="documentosLista.length === 0">
                                <td colspan="5" style="text-align:center;color:#64748b;">Sin documentos habilitados</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        const API_URL = `${window.location.origin}/public/empresa/editar_habilitacion_sifen.php`;
            ...
            return {
                ...
                async init() {
                    ...
                    if (DATOS_INICIALES) {
                        ...
                        this.documentosLista = (DOCUMENTOS_INICIALES || []).map(d => ({
                            ...d,
                            cajas: Array.isArray(d.cajas) ? d.cajas : []
                        }));
                ...
                async cargarHabilitacion() {
                    try {
                        ...
                        if (data.success && data.data) {
                            ...
                            this.documentosLista = (data.data.documentos || []).map(d => ({
                                ...d,
                                cajas: Array.isArray(d.cajas) ? d.cajas : []
                            }));
                ...
                async cargarDocumentos(id) {
                    ...
                    const data = await res.json();
                    if (data.success) {
                        this.documentosLista = (data.data || []).map(d => ({
                            ...d,
                            cajas: Array.isArray(d.cajas) ? d.cajas : []
                        }));
                    }
                ...
                async cargarCajasEmpresa() {
                    try {
                        const res = await fetch(`${API_URL}?action=list_cajas_empresa&id_empresa=${ID_EMPRESA_URL}`);
                        const data = await res.json();
                        this.cajasDisponibles = data.success ? (data.data || []) : [];
                    } catch (e) {
                        this.cajasDisponibles = [];
                    }
                },

                normalizarCod3(valor) {
                    const n = parseInt(String(valor ?? '').replace(/\D/g, ''), 10);
                    if (Number.isNaN(n)) return '';
                    return String(n).padStart(3, '0');
                },

                cajasFiltradasPorDocumento(doc) {
                    const estDoc = this.normalizarCod3(doc?.codigo_establecimiento);
                    const ptoDoc = this.normalizarCod3(doc?.punto_expedicion);
                    if (!estDoc || !ptoDoc) return [];
                    return (this.cajasDisponibles || []).filter(c =>
                        this.normalizarCod3(c.factura_1) === estDoc &&
                        this.normalizarCod3(c.factura_2) === ptoDoc
                    );
                },
                ...
                async agregarCajaDocumento(doc) {
                    const cajaRaw = (this.docCajaInput[doc.id] || '').trim();
                    const caja = cajaRaw.replace(/[^a-zA-Z0-9_\-]/g, '').substring(0, 20);
                    if (!caja) {
                        this.showToast('Ingrese una caja válida', 'warning');
                        return;
                    }
                    try {
                        const res = await fetch(`${API_URL}?action=add_documento_caja`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                        ...
                        const data = await res.json();
                        if (!data.success) {
                            this.showToast(data.error || 'No se pudo agregar la caja', 'error');
                            return;
                        }
                        this.docCajaInput[doc.id] = '';
                        await this.cargarDocumentos(this.formData.id);
                    } catch (e) {
                        this.showToast('Error de conexión al agregar caja', 'error');
                    }
                },

                async agregarCajasDocumento(doc) {
                    let seleccion = this.docCajaSelect[doc.id] || [];
                    if (!Array.isArray(seleccion)) {
                        seleccion = seleccion ? [seleccion] : [];
                    }
                    if (seleccion.length === 0) {
                        this.showToast('Seleccione una o más cajas', 'warning');
                        return;
                    }
                    try {
                        const permitidas = new Set(this.cajasFiltradasPorDocumento(doc).map(c => String(c.id)));
                        for (const cajaSel of seleccion) {
                            const caja = String(cajaSel || '').trim();
                            if (!caja) continue;
                            if (!permitidas.has(caja)) continue;
                            const res = await fetch(`${API_URL}?action=add_documento_caja`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json'
                            ...
                            const data = await res.json();
                            if (!data.success) {
                                this.showToast(data.error || `No se pudo agregar caja ${caja}`, 'error');
                                return;
                            }
                        }
                        this.docCajaSelect[doc.id] = [];
                        await this.cargarDocumentos(this.formData.id);
                    } catch (e) {
                        this.showToast('Error de conexión al agregar cajas', 'error');
                    }
                },

                async quitarCajaDocumento(doc, caja) {
                    if (!confirm(`¿Quitar caja ${caja}?`)) return;
                    try {
                        const res = await fetch(`${API_URL}?action=remove_documento_caja`, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                        ...
                        const data = await res.json();
                        if (!data.success) {
                            this.showToast(data.error || 'No se pudo quitar la caja', 'error');
                            return;
                        }
                        await this.cargarDocumentos(this.formData.id);
                    } catch (e) {
                        this.showToast('Error de conexión al quitar caja', 'error');
                    }
                },
